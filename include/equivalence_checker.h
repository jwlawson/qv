...
namespace cluster{
	...
	private:
		...
		void update_mapping(const int num_rows, std::vector<int>& mappings, const int a_index,
			const int b_index) const;
		bool permutation_valid(const int a_rows, const int a_cols, std::vector<int> col_mappings,
			const IntMatrix perm) const;
		bool perm_column_valid(const int num, std::vector<int> mappings, const IntMatrix perm,
