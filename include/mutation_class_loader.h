 ...
 */
/**
 * MutationClassLoader is an iterator over all elements in the mutation class of
 * a seed matrix. If any matrix which is calculated is found to be infinite then
...
protected:
  ...
  virtual void seen_matrix(std::shared_ptr<T> matrix,
                           std::shared_ptr<T> previous, const int vertex);
  /**
