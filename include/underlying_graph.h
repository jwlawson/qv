...
template <> struct hash<cluster::UnderlyingGraph> {
 ...
 */
template <> struct hash<std::shared_ptr<cluster::UnderlyingGraph>> {
  size_t operator()(const std::shared_ptr<cluster::UnderlyingGraph> &x) const {
    return x->hash();
  }
};
/* Add equals function to std::equal_to */
template <> struct equal_to<cluster::UnderlyingGraph> {
  bool operator()(const cluster::UnderlyingGraph &lhs,
                  const cluster::UnderlyingGraph &rhs) const {
    return lhs.equals(rhs);
  }
};
template <> struct equal_to<std::shared_ptr<cluster::UnderlyingGraph>> {
