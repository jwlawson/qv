...
void
EquivQuiverMatrix::set_matrix(const IntMatrix& mat) {
...
bool
EquivQuiverMatrix::equals(const IntMatrix& mat) const {
  auto e_mat = static_cast<EquivQuiverMatrix>(mat);
  return checker_->are_equivalent(*this, e_mat);
}

bool
EquivQuiverMatrix::equals(const EquivQuiverMatrix& mat) const {
  return checker_->are_equivalent(*this, mat);
}

void
EquivQuiverMatrix::reset() {
...
std::size_t
EquivQuiverMatrix::compute_hash() const {
  std::size_t hash = 137;
  sorted_rows_ = rows_;
