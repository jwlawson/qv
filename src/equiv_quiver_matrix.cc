...
EquivQuiverMatrix::EquivQuiverMatrix()
    : QuiverMatrix(), rows_(), checker_(EquivalenceChecker::Get(0)),
      sorted_rows_() {}

EquivQuiverMatrix::EquivQuiverMatrix(const int rows, const int cols)
    : QuiverMatrix(rows, cols), rows_(rows, std::make_pair(0, 0)),
      checker_(EquivalenceChecker::Get(rows)), sorted_rows_(rows_) {}

EquivQuiverMatrix::EquivQuiverMatrix(const int rows, const int cols,
                                     const int values[])
    : QuiverMatrix(rows, cols, values), rows_(rows, std::make_pair(0, 0)),
      checker_(EquivalenceChecker::Get(rows)), sorted_rows_(rows_) {
  reset();
}

EquivQuiverMatrix::EquivQuiverMatrix(const IntMatrix &matrix)
    : QuiverMatrix(matrix), rows_(matrix.num_rows(), std::make_pair(0, 0)),
...
EquivQuiverMatrix::EquivQuiverMatrix(std::string const &str)
    : QuiverMatrix(str), rows_(num_rows(), std::make_pair(0, 0)),
      checker_(EquivalenceChecker::Get(num_rows_)), sorted_rows_(rows_) {
  reset();
}
