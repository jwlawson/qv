...
MoveGraph<M>::_GraphLoader::operator++() {
  ...
  for (auto move_it = _graph._moves.begin(); move_it != _graph._moves.end();
       ++move_it) {
    AVec applicable =
        move_it->applicable_submatrices(std::make_shared<IntMatrix>(*mat));
    for (auto app_it = applicable.begin(); app_it != applicable.end();
         ++app_it) {
...
MoveGraph<M>::add_ss_equiv(CMatrixPtr new_mat) {
  ...
  while (!queue.empty()) {
    ...
    for (auto it = applicable.begin(); it != applicable.end(); ++it) {
...
template <class M>
typename MoveGraph<M>::CMatrixPtr
MoveGraph<M>::ss_move_equiv(CMatrixPtr chk_mat) {
  ...
  while (!queue.empty()) {
    ...
    queue.pop_front();
    auto applicable = move.applicable_submatrices(*mat);
    for (auto it = applicable.begin(); !result && it != applicable.end();
         ++it) {
      MatrixPtr pt = new M(mat->num_rows(), mat->num_cols());
      move.move(*it, *pt);
