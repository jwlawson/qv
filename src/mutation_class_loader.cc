...
template <class T>
std::shared_ptr<T>
MutationClassLoader<T>::next_ptr() {
...
template <class T>
bool
MutationClassLoader<T>::has_next() {
  return !queue_.empty();
}

template <class T>
void
MutationClassLoader<T>::calc_more(const bool calc) {
  should_calc_ = calc;
}

template <class T>
bool
MutationClassLoader<T>::compute_mutations(std::shared_ptr<T> mat) {
...
template <class T>
bool
MutationClassLoader<T>::is_complete(std::shared_ptr<T> matrix) {
  return map_[matrix].is_complete();
}

template <class T>
void
MutationClassLoader<T>::remove_handled(std::shared_ptr<T> matrix) {
  map_.erase(matrix);
}

template <class T>
void
MutationClassLoader<T>::remove_complete(std::shared_ptr<T> matrix) {
  map_.erase(matrix);
}

template <class T>
void
MutationClassLoader<T>::seen_matrix(std::shared_ptr<T> matrix,
...
template <class T>
void
MutationClassLoader<T>::unseen_matrix(std::shared_ptr<T> matrix,
                                      std::shared_ptr<T> previous,
                                      const int vertex) {
...
template <class T>
bool
MutationClassLoader<T>::have_seen(std::shared_ptr<T> matrix) {
  return map_.count(matrix) > 0;
}

template <class T>
bool
MutationClassLoader<T>::mutate_at(std::shared_ptr<T> matrix, const int i) {
