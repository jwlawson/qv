...
StreamIterator<T>::StreamIterator()
    : has_next_(false), str_(), input_(&std::cin, NullDeleter()) {}
...
template <class T> std::shared_ptr<T> StreamIterator<T>::next() {
...
template <class T> bool StreamIterator<T>::has_next() { return has_next_; }
