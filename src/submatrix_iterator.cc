...
template <class T>
SubmatrixIterator<T>::SubmatrixIterator() : count_(0), max_(0) {}

template <class T>
SubmatrixIterator<T>::SubmatrixIterator(const T &matrix)
    : matrix_(matrix),
      submatrix_(matrix_.num_rows() - 1, matrix_.num_cols() - 1), count_(0),
      max_(matrix.num_rows()) {}

template <class T> T const &SubmatrixIterator<T>::next() {
  matrix_.subquiver(count_, submatrix_);
  ++count_;
  return submatrix_;
}

template <class T> bool SubmatrixIterator<T>::has_next() {
  return count_ < max_;
}
